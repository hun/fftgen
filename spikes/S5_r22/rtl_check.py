...
def write_r22_twiddle_mem(cfg, path):
    ...
    if cfg.num_stages % 2 == 1:
        # leftover last stage (odd n): the plain D=1 stage reads W^0
        re, im = tw[0]
        words.append(((re & ((1 << cfg.twiddle_width) - 1))
                      << cfg.twiddle_width)
                     | (im & ((1 << cfg.twiddle_width) - 1)))
...
#define TB_OUTPUT_WIDTH 16
    ...
    while (cycle < TB_CYCLES) {
        ...
        dut->ce = 1;
        dut->clk = 1; dut->eval();
        // sign-extend the OUTPUT width for printing
...
def run_rtl(cfg, num_frames=2, seed=7):
    ...
    # expected (contract): the model's full streaming pipeline
    # (R2^2 chain + streaming leftover + output quantization)
    m = R22SDFGoldenModel(cfg)
    expq = m.process_stream(samples)
    ...
    cmd = ["verilator", "--cc", "--exe", "--build", "-j", "4",
           ...
           *gargs,
           "fft_r22_top.v", "fft_stage_r22.v", "fft_sdf.v", "tb_driver.cpp"]
    ...
    # the RTL emits the quantized output stream
    actq = act
