...
def _leftover_preload(cfg):
    ...
    the RTL's 3D+9 stage latencies -- 3D+9 = odd iff D is even; the
    leftover warmup parity follows the chain latency mod 2)."""
    ...
    chain_mod2 = sum(3 * (N >> (2 * t + 2)) + 9 for t in range(n // 2)) % 2
...
def top_rtl(cfg):
    ...
    for g in range(npairs):
        ...
        up_lat = sum(3 * (N >> (2 * t + 2)) + 9 for t in range(g))
        k_pre = (-up_lat) % (4 * D)
...
"""
    lat = sum(3 * (N >> (2 * t + 2)) + 9 for t in range(npairs))
    if leftover:
        lat += 11
    return src, lat
