...
`timescale 1ns/1ps
module tb_lpair #(parameter KP = 10, parameter D = 4, parameter STR = 9);
    ...
    fft_stage_r22 #(
        ...
        .K_PRELOAD(16'd0 + KP),
        .TWIDDLE_FILE("fft_tw_r22_lx.mem")
    ) u_l ( .clk(clk), .ce(ce), .rst(rst),
        .in_re(in_re), .in_im(in_im), .out_re(out_re), .out_im(out_im) );

    always #5 clk = ~clk;
    initial begin
        $readmemh("stim_lpair.mem", stim);
        $readmemh("fft_tw_r22_lx.mem", rommem);
        fd = $fopen("lp.hex", "w");
