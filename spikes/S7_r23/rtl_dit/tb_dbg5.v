...
module tb;
    ...
    reg clk = 0, rst = 1;
    reg tvalid = 1;
    reg [2*W-1:0] stim [0:1023];
    integer c;
    integer fd;
    wire signed [W-1:0] t0_re, t0_im, t1_re, t1_im, t2_re, t2_im;
    fft_sdf_r23_dit #(
        .NUM_POINTS(512), .SAMPLE_WIDTH(16), .INTERN_WIDTH(16),
        .TWIDDLE_WIDTH(18), .TWIDDLE_DECIMAL(17),
        .SCALING_PACK(64'h015555), .INVERSE(1),
    ...
    initial begin
        ...
        fd = $fopen("dbg5_out.txt", "w");
        repeat (2) @(negedge clk);
        rst = 0;
        for (c = 0; c < 600; c = c + 1) begin
            @(posedge clk); #1;
            $fwrite(fd, "%0d %h %h %h %h %h %h\n", c, t0_re, t0_im, t1_re, t1_im, t2_re, t2_im);
            @(negedge clk);
        end
        $fclose(fd);
        $finish;
