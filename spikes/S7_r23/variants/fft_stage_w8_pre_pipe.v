...
module fft_stage_r22 #(
    ...
    localparam integer W45 = CB + 1 + TWIDDLE_WIDTH;
    function signed [W45-1:0] mul_q8o;
    ...
    wire signed [W45-1:0] r45a_in = $signed({{(W45-CB){m_re[CB-1]}}, m_re}) + $signed({{(W45-CB){m_im[CB-1]}}, m_im});
    wire signed [W45-1:0] r45b_in = $signed({{(W45-CB){m_im[CB-1]}}, m_im}) - $signed({{(W45-CB){m_re[CB-1]}}, m_re});
    (* use_dsp = "no" *) wire signed [W45-1:0] r45a_q = mul_q8o(r45a_in);
    (* use_dsp = "no" *) wire signed [W45-1:0] r45b_q = mul_q8o(r45b_in);
